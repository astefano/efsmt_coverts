...
hb, ha1, x, q = Ints('hb ha1 x q')



ciC2hh = And(And(c20,  5 >= y,  y >= 0,  y == hc,  y == ha2), Not(And(c20, c21)))
...
En = Or(And(c20,c10, False), And(c20,c11, False), And(c21,c10, -q + x >= 0, -y >= -3, -x >= -7), And(c21, c11, -y >= -3, -x >= -10), And(c20,c10, -q + x >= 0, -y >= -5, -x >= -7))
...
PosClks = And(hc >= 0, ha2 >= 0, y >= 0, r >= 0, hb >= 0, ha1 >= 0, x >= 0, q >= 0, ha1a2 >= 0, hb >= 0, hc >= 0)
Gi = And(ciC2hh, ciC1hh, absReachII, NewCond, PosClks)
safe = True
#Solving EF formula: 
EFformula = Exists([hc, ha2, hb, ha1, ha1a2, hb, hc], ForAll([c20, c21, c10, c11, y, r, x, q], Implies(Gih, And(safe, En))))
s = Solver() 
s.add(EFformula) 
print s.check() 
print s.model()
