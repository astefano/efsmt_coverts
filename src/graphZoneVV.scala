...
/* For the reference, this impl. is wrt old def of TA enabled; see notebook */
  ...
  def genZ3forPTA(s: TAS, im0: List[List[String]], portDelays: Map[String, Int], dis0: String="", defaultZ3FileName: String = "defaultZ3.py", strengthen: String = "True", defaultInvs: String = "True", isSym: Boolean = false) = {
    val galph = s.flatMap{ta => getAlph(ta)}.toSet
    //filter out any port which isn't in a ta in s
    val im = im0.map{alpha => alpha.filter(p => galph(p))}.filter(im => !im.isEmpty)
    println("im = " + im)
    ...
    val gih =  "Gih = " + "And(" + localInvNames + ", EqsS, absReachII, NewCond)"
    val gihExt =  "GihExt = " + "And(" + localInvNames + ", EqsS, EqsC, Sep, absReachII, NewCond)"
